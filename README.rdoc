...
TietoFetcher is a simple message fetcher perfect for using in a daemon or via cron.
...
As Ruby 1.9 onwards use native threads SystemTimer gem becomes redundant, and timeout.rb is used instead.

== Usage

Create a new fetcher object like the following:

  @tieto_fetcher = TietoFetcher.create({:type => :pop,
...
The following options can be passed to the <tt>TietoFetcher.create</tt> factory method:
...
The TietoFetcher plugin comes with a generator to create a daemon:

  script/generate tieto_fetcher_daemon MailerDaemon

You should monitor the daemon using monit or god to ensure it does not crash.

== Running via cron

You can also run the TietoFetcher periodically via cron. It is important to ensure that only one
...
Here is an example script to be with <tt>script/runner</tt> via cron:
  ...
  begin
    Lockfile.new('cron_mail_tieto_fetcher.lock', :retries => 0) do
      config = YAML.load_file("#{RAILS_ROOT}/config/mail.yml")
      config = config[RAILS_ENV].to_options

      tieto_fetcher = TietoFetcher.create({:receiver => MailReceiver}.merge(config))
      tieto_fetcher.fetch
    end
...
Contributed by Mekšs Artūrs. Moved to gem by Mārtiņš Zaķis.
